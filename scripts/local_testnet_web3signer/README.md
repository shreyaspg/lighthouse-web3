# Simple Local Testnet with Web3Signer

These scripts allow for running a small local testnet with multiple beacon nodes and validator clients.
This setup can be useful for testing and development.

The web3signer is set up for the first VC (node_1) only.
This was primarily written to compare memory usage between VCs using a remote signer vs a local signer.
...
`wget`, `jq` and `unzip` also need to be installed on the machine.
```bash
apt install wget unzip jq
```
...
Download and run the latest web3signer
```bash
./web3signer
```
